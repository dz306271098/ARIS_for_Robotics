...
如果你是在维护主线，而不是单纯使用主线，除了看架构说明，还应该把下面这份“主线不回退基线”一起作为对照表：

- [`docs/CODEX_MAINLINE_PARITY_RULES_CN.md`](docs/CODEX_MAINLINE_PARITY_RULES_CN.md)

它不是另一份使用手册，而是把“旧主线语义回填”的最终规则压缩成一页审查基线，方便你以后判断主线有没有又滑回旧语义。
...
```bash
bash scripts/install_codex_claude_mainline.sh --reinstall --review-model claude-opus-4-6[1m]
```
...
如果你改的是主线 skill、overlay、安装器、主线 README 或审稿 guide，不要只跑 smoke test，还要按下面这份迁移说明做一次“不回退”审查：
