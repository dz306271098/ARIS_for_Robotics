...
这条主线现在默认仍对 `python_ml` 友好，但已经通过 `CODEX.md -> ## Execution Profile` 接入了 compiled / robotics 分支。也就是说，同一套 workflow 现在可以同时覆盖：

- Python/ML 训练型研究项目
- CMake-first 的 C++ 算法 / benchmark / library 论文项目
- single-node 的 CMake CUDA / CPU+CUDA 混合项目
- offline / sim / rosbag 边界内的 C++ 机器人、视觉、SLAM、感知研究项目
- 以后继续扩展的混合执行项目
...
如果你是 **C++ 算法 / benchmark 项目**，主线现在也走同一套 workflow，只需要把 `Execution Profile` 切到 compiled mode。`GPU Configuration` 里的 `gpu: local | remote` 在这些模式下表示**执行主机类型**，不表示必须有 GPU：

```markdown
## GPU Configuration
- gpu: remote
- ssh_alias: cpu-benchmark-host
- code_dir: ~/my-cpp-project
...
如果你是 **CPU+CUDA 混合项目**，就在同一个主线里继续写 CUDA 配置，不需要另开 workflow：
...
如果你是 **机器人 / 视觉 / SLAM / 感知项目**，首批主线边界固定为 offline / sim / rosbag，不自动越过到真实机器人：

```markdown
## GPU Configuration
- gpu: remote
- ssh_alias: slam-host
- code_dir: ~/my-slam-project
...
- robotics_domain: slam_perception
- data_backend: rosbag
- benchmark_suite: euroc / tum / kitti / nuscenes
...
- `experiment-bridge` / `run-experiment` 会按 `configure -> build -> ctest -> benchmark/profile` 或 `configure -> build -> ctest -> offline replay/eval` 理解执行路径
- `monitor-experiment` 会优先看 build log、CTest、benchmark JSON/CSV、trajectory/perception summary 和 profiler 工件
- `result-to-claim` 会按 correctness、runtime、memory、scaling、ATE/RPE、latency/FPS、mAP 等证据来判断 claim，而不是默认按训练曲线/W&B 读结果
- `require_wandb_for_unattended_training` 只约束训练型项目；`cpu_benchmark`、`cpu_cuda_mixed`、`slam_offline` 项目默认把 W&B 视为可选
- `allow_auto_real_robot: false` 继续是硬边界；offline / sim / rosbag 结果不能被写成真实机器人结论
...
仓库里的 `fixtures/` 是主线 smoke/runtime 的真实测试资产，应该纳入版本控制；不要再把整个 `fixtures/` 目录当成“本机临时文件”忽略掉。

相反，下面这些属于本机运行残留，不应该进入版本控制：

- 根目录 `.codex/`
- 根目录 `build/`、`install/`、`log/`
- `fixtures/*` 里的运行产物目录，例如 `build/`、`install/`、`log/`、`results/`、`monitoring/`、`profiles/`

推荐优先用仓库脚本或临时目录跑验证；如果你直接在 fixture 目录里跑 CMake / colcon / runtime check，这些生成目录会被 `.gitignore` 吸收，但 fixture 源码本身仍然应该保持可跟踪。
