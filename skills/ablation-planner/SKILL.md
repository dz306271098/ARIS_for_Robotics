...
```bash
codex exec --sandbox read-only -m gpt-5.4 "You are a rigorous ML reviewer planning ablation studies. Read the project files directly.
...
- **Codex CLI leads the design. CC does not pre-filter or bias the ablation list** before Codex sees it. Codex thinks like a reviewer; CC thinks like an engineer.
