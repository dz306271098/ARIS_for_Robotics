...
After **every** `codex exec` or `codex exec resume --last` call that serves a reviewer/critique function. This includes review scoring, experiment auditing, claim verification, idea critique, and patch gating.
...
After each reviewer Codex CLI call, save the trace using the helper script:
