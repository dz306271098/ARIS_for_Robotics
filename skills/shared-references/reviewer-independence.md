See also `codex-context-integrity.md` for the full three-tool decision tree and anti-framing self-check. This document focuses specifically on content isolation between executor and reviewer.
...
- Any skill that sends artifacts to `codex exec` or `codex exec resume --last`

## Exception

Multi-round review within the SAME thread (`codex exec resume --last`) may reference the reviewer's own previous feedback to check resolution — but still must not include executor interpretations of that feedback.
