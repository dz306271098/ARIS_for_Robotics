...
- MAX_REVIEW_ROUNDS = 3
- REVIEWER_MODEL = `gpt-5.4` via Codex CLI, reasoning effort always `xhigh`
...
Follow `../shared-references/codex-context-integrity.md` for all Codex CLI interactions.
...
3. All big-O/Theta/o statements have declared parameter dependence and uniformity scope
...
| **QUANTIFIER_ERROR** | Wrong order for-all/exists, missing "for sufficiently small kappa" | "For all pi, there exists epsilon" vs "there exists epsilon for all pi" |
...
| **LOGICAL_GAP** | A step is not justified by what precedes it | B=Theta(1) -> beta_K=0 without analyzing W |
...
| **STOCHASTIC_MODE_CONFUSION** | Mixes a.s./in prob./in L-squared/in expectation | |
...
| **DIMENSION_TRACKING** | Parameter dependence (d, n, K, ...) not explicit | d enters only through kappa |
...
| Category | Description | Example |
|----------|-------------|---------|
| **SCOPE_OVERCLAIM** | Conclusion stated more broadly than proof supports | "beta_K=0" with only generic overlap |
...
```
kappa : scalar in (0,1), depends on (d, alpha_t, Sigma, mu)
u* : vector in R^d, u* = C^{-1}m
B^even : matrix in R^{(L+1)x(L+1)}, symmetric PSD
Psi_v : function R -> R, analytic in (zeta,kappa), parity determined by v
```
...
```
For all K >= 3, for all pi in Pi_K^{ms,o} \ E_K, exists kappa_0 > 0 such that for all kappa in (0, kappa_0):
  h_act^{(K,pi)} = Theta(kappa^{alpha_K^act})  [uniform in pi on compact subsets]
```
...
```
MC-17: Context: [Lemma 3.1, kappa < kappa_0, Z_kappa has bounded moments up to order 2m+2]
       |- Goal: P_hat_0 is positive definite
...
```
h_act = Theta(kappa^alpha)  [as kappa->0, uniform in pi on compact subsets of Pi_K, for fixed K]
tau_act ~ (b/a)n   [as n->inf, for fixed kappa,K,pi with x_K << 1]
```
...
F. UNIFORMITY & CONSTANTS: For every O(.), o(.), Theta(.), <~, state whether
...
Parse into structured issue list. Write to `PROOF_AUDIT.md`.
...
Invoke the rescue subagent for adversarial counterexample search:
...
Use `codex exec resume --last` with the previous Codex thread. Include fix summaries. Request the same mandatory checklist.
...
For any fix that resolved a FATAL or CRITICAL issue, submit the **fixed section alone** (without showing the previous critique) to a **fresh Codex thread** (NOT resume):
...
- **Uniformity discipline**: Every O(.)/Theta(.) must declare what parameters it is uniform over. "O(1)" that secretly depends on d,n,K is a CONSTANT_DEPENDENCE_HIDDEN issue.
- **Quantifier discipline**: Check for-all/exists order. "For sufficiently small kappa" must specify: does kappa_0 depend on K? On pi? On d?
...
- **Model used via Codex CLI**: GPT-5.4 xhigh through `codex exec`.
- **Codex reasoning always xhigh**: Never downgrade.
- **Send full content**: Don't summarize -- send actual math for line-by-line checking.
- **Use resume for follow-ups**: Use `codex exec resume --last` for re-review rounds within the same session.
