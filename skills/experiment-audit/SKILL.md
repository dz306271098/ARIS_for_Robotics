...
### Step 2: Send to Reviewer (GPT-5.4 via Codex CLI)

Pass ONLY file paths and the audit checklist to the reviewer. The reviewer reads everything directly.

**CRITICAL: Always use a fresh `codex exec` call. NEVER use `codex exec resume --last`. Every audit run must be a fresh context with zero prior information.**
...
3. Does the claimed NUMBER match what is in the file?
...
**Date**: [today]
**Auditor**: GPT-5.4 xhigh (cross-model, read-only, via Codex CLI)
...
Experiment Audit Complete
  ...
  Score Normalization: WARN -- boundary metric uses self-reference
  Result Existence:    PASS -- all files exist, numbers match
  Dead Code:           PASS -- all metric functions called
  Scope:               WARN -- 2 scenes, paper says "comprehensive"
