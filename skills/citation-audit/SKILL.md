...
allowed-tools: Bash(*), Bash(codex*), Read, Grep, Glob, Edit, Write, Agent, WebSearch, WebFetch, Skill(codex:rescue)
---
...
- **REVIEWER_MODEL = `gpt-5.4`** — Used via Codex CLI (`codex exec --sandbox read-only -m gpt-5.4`). Default for cross-model review with web access.
- **CONTEXT_POLICY = `fresh`** — Each audit run uses a new Codex session (REVIEWER_BIAS_GUARD). Never `codex exec resume --last`.
...
- **STATE = `CITATION_AUDIT.json`** — Machine-readable verdict ledger consumable by `tools/verify_paper_audits.sh`.
...
For each bib entry, invoke `codex exec` with a fresh session (NOT `codex exec resume --last` — fresh thread per entry, or batch with explicit per-entry isolation):

```bash
codex exec --sandbox read-only -m gpt-5.4 \
  --output-schema "$SCHEMA_PATH/citation-audit-entry.schema.json" \
  -o /tmp/aris-citation-entry-$KEY.json \
  "You are auditing a bibliographic entry. Use web/DBLP/arXiv search.

## Bib entry
[Paste the BibTeX entry verbatim]

## Where this entry is cited in the paper
[Paste extracted contexts from .aris/citation-audit/contexts.txt]
...
Read the paper files directly: main.tex, references.bib, and every sections/*.tex referenced above."
```
...
- **Fresh reviewer session per audit run** — never use `codex exec resume --last` for citation audits
...
After each `codex exec` reviewer call, save the trace following `shared-references/review-tracing.md`. Use `bash tools/save_trace.sh` or write files directly to `.aris/traces/citation-audit/<date>_run<NN>/`. Respect the `--- trace:` parameter (default: `full`).
...
Every invocation uses a fresh `codex exec` session. Never `codex exec resume --last`. Do not accept prior audit outputs (PROOF_AUDIT, PAPER_CLAIM_AUDIT, EXPERIMENT_LOG) as input — the fresh session preserves reviewer independence per `shared-references/reviewer-independence.md`.
...
- `shared-references/integration-contract.md` — architectural contract under which this skill operates
