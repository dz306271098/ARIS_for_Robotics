...
- **REVIEWER_MODEL = `gpt-5.4`** — Model used via Codex CLI for outline review. Must be an OpenAI model.
...
Send the complete outline to GPT-5.4 for feedback:

```bash
codex exec --sandbox read-only -m gpt-5.4 "Read NARRATIVE_REPORT.md and experiment results directly. Review the paper outline for: claims-evidence completeness, section balance, citation coverage. Read the project files directly for ground truth."
```
