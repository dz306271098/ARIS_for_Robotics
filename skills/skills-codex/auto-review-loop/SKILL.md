...
description: Autonomous multi-round research review loop. Repeatedly reviews using a secondary Codex agent, implements fixes, validates them, and re-reviews until positive assessment or max rounds reached. Use when user says "auto review loop", "review until it passes", or wants autonomous iterative improvement.
...
- **REVIEWER_MODEL = `gpt-5.4`** — Model used via a secondary Codex agent for strict external review.
