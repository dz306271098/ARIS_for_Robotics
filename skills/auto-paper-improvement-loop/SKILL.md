...
- **REVIEWER_BIAS_GUARD = true** — When `true`, every review round uses a fresh `codex exec` invocation with no prior review context. Never use `codex exec resume --last` for review rounds. Set to `false` only for deliberate debugging of the legacy behavior. **Empirical evidence (April 2026):** running the same paper with `resume --last` + "since last round we did X" prompts inflated scores from real 3/10 → fake 8/10 across 5 rounds; switching to fresh threads recovered the true 3/10 assessment.
...
See also: `../shared-references/reviewer-independence.md` and `../shared-references/codex-context-integrity.md`.

Rules:
- Every round starts with a fresh `codex exec` invocation, not `codex exec resume --last`.
- Never pass prior review context into the next review prompt.
...
```bash
codex exec --sandbox read-only -m gpt-5.4 "You are reviewing a [VENUE] paper. Please provide a detailed, structured review.
...
def extract_theorems(text):
    """Extract theorem/lemma/proposition/corollary statement blocks."""
    pattern = r'\\begin\{(theorem|lemma|proposition|corollary)\}(.*?)\\end\{\1\}'
    blocks = re.findall(pattern, text, re.DOTALL)
    result = {}
    for env_type, body in blocks:
        # Extract label if present
        label_match = re.search(r'\\label\{([^}]+)\}', body)
        key = label_match.group(1) if label_match else f"{env_type}_{len(result)}"
        result[key] = normalize(body)
    return result

# Classify files: before \appendix = main body, after = appendix
main_theorems, appendix_theorems = {}, {}
for tex_file in sorted(glob.glob('paper/sections/*.tex')):
    with open(tex_file, 'r', errors='ignore') as f:
        content = f.read()
    is_appendix = 'appendix' in tex_file.lower() or '\\appendix' in content
    target = appendix_theorems if is_appendix else main_theorems
    target.update(extract_theorems(content))

# Compare main body vs appendix restatements
mismatches = []
for key in main_theorems:
    if key in appendix_theorems:
        if main_theorems[key] != appendix_theorems[key]:
            mismatches.append(f"DRIFT: {key}\n  Main: {main_theorems[key][:100]}...\n  Appendix: {appendix_theorems[key][:100]}...")

if mismatches:
    print("RESTATEMENT REGRESSION DETECTED — BLOCKING:")
    for m in mismatches:
        print(f"  {m}")
    sys.exit(1)
else:
    print(f"OK: {len(main_theorems)} main-body theorems, {len(appendix_theorems)} appendix theorems, 0 mismatches")
PY
```
...
If `REVIEWER_BIAS_GUARD = true` (default), use a **fresh** `codex exec` invocation (see Reviewer Independence Protocol above).

```bash
codex exec --sandbox read-only -m gpt-5.4 "You are reviewing a [VENUE] paper. This is a fresh, zero-context review.
...
If `REVIEWER_BIAS_GUARD = false` (legacy debugging only), use `codex exec resume --last` with Round 1 context; this is **not** the recommended path.
...
This is a late-stage adversarial check. It must always use **fresh** `codex exec` invocations, and it must not reuse any prior review context.

**Thread 1: Attack**
- Use a fresh `codex exec` with only the current paper files.
- Prompt:
```bash
codex exec --sandbox read-only -m gpt-5.4 "Construct the single best argument to reject this paper in 200 words. Focus on theorem validity, assumption mismatch, missing proof obligations, limit-order ambiguity, and claim/evidence gaps. Do not reference prior rounds or fixes. Read paper/sections/*.tex and paper/main.pdf directly."
```

**Thread 2: Defense**
- Use a second fresh `codex exec` with the current paper files plus the attack memo.
- Prompt:
```bash
codex exec --sandbox read-only -m gpt-5.4 "Now defend the paper against the following attack memo. For each rejection point, classify it as already fixed, partially fixed, or still unresolved, and cite the current files. Do not reuse prior review context. Read paper/sections/*.tex and paper/main.pdf directly.

ATTACK MEMO:
[paste attack memo from Thread 1]"
```
...
Run Step 4.5 (Restatement Regression Test) again after this recompile.
...
- **Reviewer independence (Round 2+)**: see Reviewer Independence Protocol section above
...
After each `codex exec` reviewer call, save the trace following `../shared-references/review-tracing.md`. Use `bash tools/save_trace.sh` or write files directly to `.aris/traces/auto-paper-improvement-loop/<date>_run<NN>/`. Respect the `--- trace:` parameter (default: `full`).
