...
description: "Autonomous multi-round research review loop. Repeatedly reviews using Gemini CLI via gemini-review MCP, implements fixes, validates them, and re-reviews until positive assessment or max rounds reached. Use when user says \"auto review loop\", \"review until it passes\", or wants autonomous iterative improvement."
...
```
mcp__gemini_review__review_reply_start:
  threadId: [saved completed reviewer `threadId`]
  prompt: |
