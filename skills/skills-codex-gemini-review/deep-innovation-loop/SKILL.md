...
| `TARGET_SCORE` | 8 | Primary stop condition (Gemini review score for target venue) |
...
| `REVIEWER_MODEL` | gpt-5.4 | External reviewer model via the local `gemini-review` MCP bridge |
...
Append these independent findings to the collaborative context below and save the completed reviewer `threadId` for follow-up.

**Step 1: Collaborative reanalysis** (multi-turn, with the saved reviewer thread):
```
mcp__gemini_review__review_reply_start:
  threadId: [saved completed diagnosis `threadId`]
  prompt: |
...
**CRITICAL**: Save the FULL raw response verbatim. Save the completed reviewer `threadId` from the response so later follow-ups can reference it if needed.
...
```
mcp__gemini_review__review_reply_start:
  threadId: [saved completed innovation-design `threadId`]
  prompt: |
...
2. Did it survive the Gemini reviewer's adversarial critique without a fatal flaw?
...
- If verdict = `needs-attention` → apply the shared **Reviewer Resolution Protocol** (see `../shared-references/reviewer-resolution-protocol.md`):
  ...
  - Disputed findings → submit rebuttal to the same reviewer thread for adjudication:
    ```
mcp__gemini_review__review_reply_start:
  threadId: [saved completed implementation-audit `threadId`]
  prompt: |
...
2. **Generate 2-3 loss variants** via the local `gemini-review` MCP bridge:
...
- Each round should use a fresh reviewer call for independent diagnosis; use `mcp__gemini_review__review_reply_start` plus `mcp__gemini_review__review_status` only when continuity materially helps the next review step
