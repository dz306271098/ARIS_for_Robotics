...
refine-logs/EXPERIMENT_PLAN.md   →   implement → Gemini review → deploy → collect → initial results ready
...
- **CODE_REVIEW = true** — Gemini CLI review reviews experiment code before deployment. Catches logic bugs before wasting GPU hours. Set `false` to skip.
...
- **REVIEWER_RESOLUTION_PROTOCOL = true** — Disputed reviewer findings must be pushed back through the same reviewer thread until they are accepted, narrowed, rebutted, or resolved via a minimum action. See `../shared-references/reviewer-resolution-protocol.md`.
