...
description: "Use when main results pass result-to-claim (`claim_supported = yes` or `partial`) and ablation studies are needed for paper submission. A Gemini reviewer via gemini-review MCP designs ablations from a reviewer's perspective; the local executor reviews feasibility and implements."
...
### Step 2: Gemini Reviewer Designs Ablations
