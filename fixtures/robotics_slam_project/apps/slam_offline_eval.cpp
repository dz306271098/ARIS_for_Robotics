...
int main(int argc, char** argv) {
    ...
    if (trajectory_output.empty() || perception_output.empty()) {
        std::cerr << "Usage: slam_offline_eval --trajectory-output <path> --perception-output <path>" << std::endl;
        return 1;
    }

    const std::vector<double> reference{0.0, 1.0, 2.1, 3.2, 4.0, 5.2};
    const std::vector<double> estimate{0.0, 1.05, 2.0, 3.3, 3.95, 5.1};
    ...
    const double drift_percent = ate * 12.0;
    const double fps = 47.5;
    const double latency_ms = 1000.0 / fps;

    const double precision = 0.920000;
    const double recall = 0.885000;
    const double map = 0.903000;
    const double perception_fps = 38.0;
    const double perception_latency_ms = 1000.0 / perception_fps;
    const int true_positive = 48;
    const int false_positive = 5;
    const int false_negative = 7;
    ...
    perception_stream << "    \"missed_objects\": 7,\n";
    perception_stream << "    \"false_matches\": 5\n";
    perception_stream << "  }\n";
    perception_stream << "}\n";

    std::cout << "slam_offline_eval wrote summaries to " << trajectory_output << " and " << perception_output << std::endl;
    return 0;
}
